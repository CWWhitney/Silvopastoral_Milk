...
compare_NPV_Y_10ha <-  plot_distributions(mcSimulation_object = dairy_mc_simulation_10k_Y_10ha, 
                                          vars = c("NPV_no_AF", "NPV_AF"), #NPV_decision_do
                                          method = "boxplot",
  ...
  plot_distributions(mcSimulation_object = dairy_mc_simulation_10k_Y_10ha, 
                     vars = c("NPV_AF" , "NPV_no_AF"), #NPV_decision_do
...
#### decision_do ####
decision_do_NPV_Y_10ha <-  plot_distributions(mcSimulation_object = dairy_mc_simulation_10k_Y_10ha, 
                                          vars = c("NPV_decision_do"), #"NPV_no_AF", "NPV_AF"
                                          method = "boxplot",
                                          x_axis_name = "",
                                          y_axis_name = "",
                                          base_size = 11
) +
  labs(title = "Distribution of Net Present Values of Introducing Agroforestry System\n(with Ecological Aspects)"
  ) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        plot.margin = margin(t = 0)
  ...
  plot_distributions(mcSimulation_object = dairy_mc_simulation_10k_Y_10ha, 
                     vars = c("NPV_decision_do"), #"NPV_AF" , "NPV_no_AF"
                     method = "hist_simple_overlay", 
                     bins = 200,
                     binwidth = NULL,
                     x_axis_name = "Net Present Value (EUR)",
                     y_axis_name = "Frequency",
                     base_size = 11
  )+
  guides(fill = guide_legend(title = "Decision Options")
  ) +
  scale_fill_manual(labels=c("Introducing\nShort-Rotation\nPlantations\nof Poplars in to\nGrazing Grassland\nCompared to\nConventional\nGrazing Grassland\n(Decision Do)"),
                    values=c("#009999", "#0000FF")
  )+
  theme( legend.justification = c(0.3, 0.7),
         legend.location = "plot"
...
decision_do_NPV_Y_10ha


ggsave(filename = "images/decision_do_NPV_003Y_10ha_english.png", 
       plot = decision_do_NPV_Y_10ha, 
       width = 9, 
       height = 3
)
...
#### decision_do ####
decision_do_NPV_N_10ha <-  plot_distributions(mcSimulation_object = dairy_mc_simulation_10k_N_10ha, 
                                              vars = c("NPV_decision_do"), #"NPV_no_AF", "NPV_AF"
                                              method = "boxplot",
                                              x_axis_name = "",
                                              y_axis_name = "",
...
decision_do_NPV_N_10ha


ggsave(filename = "images/decision_do_NPV_003N_10ha_english.png", 
       plot = decision_do_NPV_N_10ha, 
       width = 9, 
       height = 3
)
...
decision_do_cashflow_10ha <- plot_cashflow(mcSimulation_object = dairy_mc_simulation_10k_N_10ha, 
                                           ...
                                           x_axis_name = "Time after Decision (Years)",
                                           y_axis_name = "Cash Flow (EUR)",
                                           ...
                                           color_median = "blue",
                                           base_size = 11,
                                           labs(title = "Cash Flow for Introducing Short-Rotation\nPlantations of Poplars in to Grazing\nGrassland Compared to Conventional\nGrazing Grassland “Decision Do”\n(without Ecological Aspects)")
)+
...
ggsave(filename = "images/decision_do_cashflow_003_10ha_english.png", 
...
pls_N_10ha <- plot_pls(plsrResults = pls_result_N_10ha, 
                       input_table = data.frame(variable = input_table$variable, 
                                                label = input_table$label_english
                       ), 
                       cut_off_line = 1,
                       x_axis_name = "Importance of Variables in the Projection",
                       y_axis_name = NULL,
  ...
  labs(title = "Importance of Variables (without Ecological Aspects)")

pls_N_10ha


ggsave(filename = "images/pls_1_003N_10ha_english.png", 
       plot = pls_N_10ha, 
       width = 8, 
       height = 3
)
...
pls_Y_10ha <- plot_pls(plsrResults = pls_result_Y_10ha, 
  ...
  labs(title = "Importance of Variables (with Ecological Aspects)")

pls_Y_10ha


ggsave(filename = "images/pls_1_003Y_10ha_english.png", 
       plot = pls_Y_10ha, 
       width = 8, 
       height = 3
)
